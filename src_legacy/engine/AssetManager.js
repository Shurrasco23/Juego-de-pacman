// AssetManager: loads images and exposes sprite coordinates.
// Kept intentionally small and synchronous-friendly for learning.
export default class AssetManager {
  constructor(){
    // Image element that will contain the spritesheet
    this.spritesheet = new Image();

    // Mapping of bitmask -> tile coordinates on the spritesheet (in tiles, not pixels)
    // These values are small integers that will be multiplied by TILE_SIZE when drawing.
    this.spriteCoords = {
      0:  {x: 0, y: 0},
      1:  {x: 2, y: 3},
      2:  {x: 0, y: 3},
  ...
  // load(): returns a Promise that resolves when the spritesheet image is ready.
  load(){
    return new Promise((resolve, reject) => {
      // Set the source of the image. Path is relative to index.html location.
      this.spritesheet.src = 'Imagenes/mazeSpritesheet.png';
...
function calculateBitmask(boolMap, x, y, cols, rows){
    ...
    if ((y > 0 && boolMap[y - 1][x]) || y == 0) value += 1;          // Norte
    if ((x > 0 && boolMap[y][x - 1]) || x == 0) value += 2;          // Oeste
    if ((x < cols - 1 && boolMap[y][x + 1]) || x == cols - 1) value += 4;   // Este
    if ((y < rows - 1 && boolMap[y + 1][x]) || y == rows - 1) value += 8;   // Sur
    //Importante tomar los tiles que se encuentran en el borde externo como caminables, 
    //para invitar al jugador a usarlos
    return value;
}